...
void main() async {
  var excelFilePath = 'total_table.xlsx';
      ...
      try {
        switch (request.uri.path) {
          ...
          case '/api/0009': // Read
            ...
            break;
          case '/api/0010': // Read
            readMenuInfore(storeDb, request, recentsearchDb);
            break;
          case '/api/0011': // Read
            readStoreInfore(storeDb, request, recentsearchDb);
            break;
          default:
...
void printAndSendHttpResponse(HttpRequest request, dynamic content) async {
...
void readMenuInfo(var storeDb, var request, var recentsearchDb) async {
  ...
  storeDb.forEach((key, value) {
    if (value.isNotEmpty && value[0].toString().contains(searchWord) && value[3] >= 10) {
      findMenu[key] = [
        value[0],
        value[1],
        double.parse(value[2].toStringAsFixed(1)),
        value[3],
        double.parse(value[5].toStringAsFixed(1)),
        value[6]
      ];
    }
  });
  ...
  var sortedEntries = findMenu.entries.toList()
    ..sort((a, b) {
      ...
      return compareFirstValues == 0
          ? b.value[3].compareTo(a.value[3])
          : compareFirstValues;
    });
...
void readMenuInfore(var storeDb, var request, var recentsearchDb) async {
  var token = request.headers['authorization']![0];
  var content = await utf8.decoder.bind(request).join();
  var searchWord = jsonDecode(content);
  ...
  var sortedEntries = findMenu.entries.toList()
    ..sort((a, b) {
      // Compare the first values
      int compareFirstValues = b.value[3].compareTo(a.value[3]);
...
void readStoreInfo(var storeDb, var request, var recentsearchDb) async {
  var token = request.headers['authorization']![0];
  var content = await utf8.decoder.bind(request).join();
  var searchWord = jsonDecode(content);
  ...
  Map<int, List<dynamic>> findMenu = {};
  storeDb.forEach((key, value) {
    if (value.isNotEmpty && value[1].toString().contains(searchWord) && value[3] >= 10) {
      findMenu[key] = [
        value[0],
        value[1],
        double.parse(value[2].toStringAsFixed(1)),
  ...
  printAndSendHttpResponse(request, sendtoClient.toString());
}

void readStoreInfore(var storeDb, var request, var recentsearchDb) async {
  var token = request.headers['authorization']![0];
  var content = await utf8.decoder.bind(request).join();
  var searchWord = jsonDecode(content);
...
void createId(var userinfoDb, var request, UserNumber usernumber,
    var recentsearchDb) async {
  var content = await utf8.decoder.bind(request).join();
  dynamic decodedContent = jsonDecode(content);

  List<dynamic> convertMapToList(Map<String, dynamic> decodedContent) {
    List<dynamic> resultList = decodedContent.values.toList();
    return resultList;
  }

  dynamic transaction = convertMapToList(decodedContent);
  ...
  if (userinfoDb.isEmpty) {
    ...
    for (var key in keys) {
      ...
      if (value.isNotEmpty && value[0] == transaction[0]) {
        ...
        make = false;
        break;
      } else if (value.isNotEmpty && value[2] == transaction[2]) {
        content = "existing_nickname";
        make = false;
        break;
      } else if (value.isNotEmpty && value[3] == transaction[3]) {
        content = "existing_number";
        make = false;
        break;
      } else {
        continue;
...
void login(
    var userinfoDb, var loginInfo, var request, var recentsearchDb) async {
  var content = await utf8.decoder.bind(request).join();
  dynamic decodedContent = jsonDecode(content);
...
void getUrl(var storeDb, var request, var recentsearchDb) async {
  ...
  print("> Send to Client \n");
  printAndSendHttpResponse(request, storeDb[menutoken][4].toString());
}

void clickStar(var starDb, var request, var storeDb) async {
  var token = int.parse(request.headers['authorization']![0]);
  var tf = request.headers['tf']![0];
  var content = await utf8.decoder.bind(request).join();
  ...
  if (tf == 'true') {
    ...
    if (starDb[token] != null) {
      starDb[token].removeWhere((entry) => entry.keys.first == menutoken);
    }
  }
...
void getStar(var starDb, var request, var storeDb) async {
  ...
  printAndSendHttpResponse(request, starDb[token].toString());
}
